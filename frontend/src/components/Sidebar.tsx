...
const Sidebar = ({ fileTree, userName, userId }: SidebarProps) => {
  ...
  // Zustand Store
  const { sessions, loadHistory, loadSession, resetChat, currentSessionId } =
    useChatStore();
        ...
        <div>
          ...
          {showHistory && (
            <div className="ml-2 pl-2 border-l border-gray-300 dark:border-zinc-800 space-y-0.5 mt-1 max-h-48 overflow-y-auto scrollbar-thin scrollbar-thumb-zinc-800">
              {sessions.length === 0 ? (
                <span className="text-xs text-gray-500 dark:text-zinc-600 px-2 py-1 block italic">
                  No history yet
                </span>
              ) : (
          ...
          {showSettings && (
            <>
              ...
              <div className="absolute left-full top-0 ml-2 w-56 bg-white dark:bg-[#1a1a1a] border border-gray-200 dark:border-zinc-700 rounded-lg shadow-xl p-3 z-50">
                <p className="text-[10px] font-bold text-gray-500 dark:text-zinc-500 mb-2 uppercase tracking-wider">
                  Appearance
                </p>
                ...
                >
                  ...
                  >
                    <div
                      ...
                      style={{ left: darkMode ? "1.1rem" : "0.15rem" }}
                    ></div>
                  </div>
                </button>
              </div>
