...
const Dashboard = () => {
  ...
  const userName = session?.user?.user_metadata?.full_name || localStorage.getItem("userName") || "User";
  ...
  const repoName = repoPath ? repoPath.split(/[/\\]/).pop() : "No Repository Selected";
  ...
  return (
    ...
    <div className="flex h-screen bg-white dark:bg-[#0a0a0a] text-zinc-900 dark:text-zinc-100 overflow-hidden transition-colors duration-300 relative">
      ...
      <div
        className={`${isSidebarOpen ? 'w-64' : 'w-0'} transition-all duration-300 border-r border-gray-200 dark:border-zinc-800 bg-gray-50 dark:bg-[#111] relative`}
      >
      ...
      <div className="flex-1 flex flex-col min-w-0 relative">
        ...
        <div className="h-14 border-b border-gray-200 dark:border-zinc-800 flex items-center px-4 justify-between bg-white/80 dark:bg-[#111]/80 backdrop-blur transition-colors">
          <div className="flex items-center gap-3">
            ...
            <span className="font-semibold text-blue-600 dark:text-primary">CodeMind AI</span>
      ...
      <div
        className={`${isContextOpen ? 'w-80' : 'w-0'} transition-all duration-300 border-l border-gray-200 dark:border-zinc-800 bg-gray-50 dark:bg-[#111] overflow-hidden`}
      >
        <ContextPanel />
      </div>
